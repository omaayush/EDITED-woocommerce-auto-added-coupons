...
*/ 
	...
	public function admin_coupon_metabox_customer( $thepostid, $post ) {
		...
		// User groups
		?>
		<p class="form-field"><label><?php _e( 'Allowed Groups', 'woocommerce-jos-autocoupon' ); ?></label>
			<input class="wc-customer-group-search" data-multiple="true" style="width: 50%;" name="wjecf_customer_group_ids"   data-selected="<?php

			$coupon_customer_group_ids = WJECF()->get_coupon_customer_group_ids( $thepostid );
			$json_ids    = array();
			global $table_prefix;
			foreach( $coupon_customer_group_ids as $group_id){
				global $wpdb;
				$results=$wpdb->get_results("select name from ". $table_prefix ."groups_group where group_id='".$group_id."'",ARRAY_N);
				$json_ids[$group_id]=$results[0][0];

			}

			echo esc_attr( json_encode( $json_ids ) );
			?>" value="<?php echo implode( ',', array_values( $json_ids ) ); ?>"   /> <img class="help_tip" data-tip='<?php
			_e( 'Only these groups members may use this coupon. </br> Write Group names seperated by a single comma .', 'woocommerce-jos-autocoupon' );
			?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
	...
	public function process_shop_coupon_meta( $post_id, $post ) {
		...
		$groupnamearr=explode(',',$_POST['wjecf_customer_group_ids']);
		$tmparr=array();
		global $table_prefix;
		foreach($groupnamearr as $grp){
			global $wpdb;
			$id1=$wpdb->get_var("select group_id from ". $table_prefix ."_groups_group where name='".trim($grp)."'");
			if($id1!=null)
				array_push($tmparr,$id1);
		}
		$groupidstr=implode(',',$tmparr);
		//$wjecf_customer_group_ids = $this->comma_separated_int_array( $_POST['wjecf_customer_group_ids'] );
		update_post_meta( $post_id, '_wjecf_customer_group_ids', $groupidstr );
