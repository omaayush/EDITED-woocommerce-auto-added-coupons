...
/* FRONTEND HOOKS */
	...
	public function assert_coupon_is_valid ( $valid, $coupon ) {
		...
		$coupon_customer_group_ids = $this->get_coupon_customer_group_ids( $coupon->id );
		$coupon_customer_roles = $this->get_coupon_customer_roles( $coupon->id );
		global $table_prefix;
		if ( sizeof( $coupon_customer_ids ) > 0 || sizeof( $coupon_customer_group_ids ) > 0 || sizeof( $coupon_customer_roles ) > 0 ) {
			$user = wp_get_current_user();
			$alloweduser=array();
			global $wpdb;
			for($i=0;$i<sizeof($coupon_customer_group_ids);$i++){
				$results = $wpdb->get_results( "select user_id from ". $table_prefix ."_groups_user_group where group_id='".trim($coupon_customer_group_ids[$i])."'", ARRAY_N );
				for($j=0;$j<sizeof($results);$j++)
					array_push($alloweduser,$results[$j][0]);

			}
			//var_dump($alloweduser);
			error_log(json_encode($alloweduser));
			//If both fail we invalidate. Otherwise it's ok
			if ( ! in_array( $user->ID, $coupon_customer_ids ) && ! in_array( $user->ID, $alloweduser ) && ! array_intersect( $user->roles, $coupon_customer_roles ) ) {
 ...
 */	
	...
	public function get_coupon_customer_group_ids($coupon_id) {
		$v = get_post_meta( $coupon_id, '_wjecf_customer_group_ids', true );
		//$v = array_map( 'intval', explode(",", get_post_meta( $coupon_id, '_wjecf_customer_ids', true ) ) );
		if ($v == '') {
			$v = array();
		} else {
